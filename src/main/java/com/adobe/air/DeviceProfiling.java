...
class DeviceProfiling {
    ...
    private static void insertPreloadTagEntry() {
        ...
        String str2 = ApplicationFileManager.getAppRoot() + File.separator + META_INF + File.separator + PROFILER_SWF_NAME;
        try {
        ...
        String concat = new String("PreloadSwf=").concat(str2).concat("?");
        StringBuffer stringBuffer = new StringBuffer();
        if (hashMap.get("Host") != null) {
            str = concat.concat("host=").concat(hashMap.get("Host"));
        } else {
            str = concat;
        }
        String concat2 = str.concat("&").concat("port=").concat(DEFAULT_PROFILER_PORT);
        if (file.exists()) {
            try {
